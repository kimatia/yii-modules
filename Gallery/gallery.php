<?
// This extension includes behaviors and widgets for implementing image gallery of an ActiveRecord model. Uploading images is possible both in the widget, which is part of the standard create/update model form, and with a separate AJAX widget. Supports interactive reorder (drag and drop) and deleting of images.

// Minimum settings

// First you need to generate a table in the database. Copy or inherit migration, then execute yii migrate console command. Alternatively, you can create this table in the database manually.
...
// Saving, deleting and reordering images performs after the form has been submited. Also, the GalleryFormBehavior (this may be the ActiveRecord model to which the images are being attached, or a separate form model, depending on your application architecture) should be added to the create/update model form:
...
// Each image can be saved in several szes. By using the Imagine image manipulation library, you have full control to create your own custom image variants. The set of sizes can be changed in the basic gallery behavior settings:
...
// By default, each image is saved in two sizes - 'original' (original image file without any changes) and 'preview' (200 x 200 pixels preview). Each can be overridden in the same way as adding new sizes.
